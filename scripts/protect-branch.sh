...
if [ "$MODE" = "solo" ]; then
  curl -s -X PUT -H "Authorization: Bearer $TOKEN" -H "Accept: application/vnd.github+json" \
    ...
    -d '{"required_status_checks":{"strict":true,"contexts":["validate","lint","typecheck","test","coverage"]},"enforce_admins":true,"required_pull_request_reviews":null,"restrictions":null,"allow_force_pushes":false,"allow_deletions":false,"required_conversation_resolution":true}'
else
  curl -s -X PUT -H "Authorization: Bearer $TOKEN" -H "Accept: application/vnd.github+json" \
    "$API/repos/$OWNER/$REPO/branches/main/protection" \
    -d '{"required_status_checks":{"strict":true,"contexts":["validate","lint","typecheck","test","coverage"]},"enforce_admins":true,"required_pull_request_reviews":{"dismiss_stale_reviews":true,"require_code_owner_reviews":true,"required_approving_review_count":1,"require_last_push_approval":true},"restrictions":null,"allow_force_pushes":false,"allow_deletions":false,"required_conversation_resolution":true}'
fi
